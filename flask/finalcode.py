...
async def playwright_worker_main():
    ...
    while True:
        ...
        # No, task_done should be called by the task itself, or after it's truly handled.
        # For create_task, it's fire-and-forget from the perspective of this loop.
        # playwright_job_queue.task_done() # This is for if we `await` the task.
                    ...
                    try:
                        ...
                        if is_green_or_yellow_email_row and "@" in initial_email_cell_text and "." in initial_email_cell_text \
                           and "Add to list" not in initial_email_cell_text and "No email found" not in initial_email_cell_text:
                            ...
                            if "@" in potential_email_parsed and "." in potential_email_parsed:
                                ...
                                saved_btn_el_h = None
                                if action_cell_el_h:
                                    saved_btn_el_h = await action_cell_el_h.query_selector("xpath=.//button[contains(@class, 'snv-btn') and contains(@class, 'pl-select__top-target') and .//span[contains(text(), 'Saved')]]")
                        ...
                        if not final_email_str_from_row: # Needs "Add to list" or email wasn't G/Y
                            ...
                            if action_cell_el_h:
                                ...
                                add_list_btn_el_h = await action_cell_el_h.query_selector("xpath=.//button[contains(@class, 'snv-btn') and contains(@class, 'pl-select__top-target') and .//span[contains(text(), 'Add to list')]]")
...
if __name__ == "__main__":
    ...
    # time.sleep(1) # Optional: if issues with queue not being ready immediately
    ...
    socketio.run(
        ...
        host="0.0.0.0",
        port=8000,
        allow_unsafe_werkzeug=True, # For debug mode with reloader off
        use_reloader=False # Important: Werkzeug reloader can cause issues with threads/asyncio setup
    )
